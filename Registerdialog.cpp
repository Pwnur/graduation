    ...
    ui(new Ui::RegisterDialog)
{
    ...
    //设置对话框的大小
    this->setMaximumSize(270,380);
    this->setMinimumSize(270,380);
    ...
    //设置两个按钮的图案
    ui->registerbtn->setIcon(QIcon(":/Enter.png"));
    ui->returnbtn->setIcon(QIcon(":/return.png"));
    ...
    connect(ui->id,SIGNAL(textChanged(QString)),this,SLOT(registerbtnSetSlot(QString)));
}
...
void RegisterDialog::registerbtnSlot()
{
    ...
    if(radiogroup->checkedId()==0)
    {
        ...
        for(i=0;i<model->rowCount();i++)
        {
            ...
            //判断学生信息是否已经存在
             if((record.value(0)==ui->id->text()&&record.value(2)!=""))
            {
        ...
        if(i==model->rowCount())
        {
            ...
            record1.setValue("id",ui->id->text());
    ...
    else if(radiogroup->checkedId()==1)
    {
        ...
        if(ui->rootline->text() != j)
        {
            QMessageBox::warning(this,"警告","您没有权限",QMessageBox::Yes);
            this->clearAll();
            this->setyanzheng();
            return;
        }
