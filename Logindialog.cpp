...
/*
    //设置登录对话框的图标
    this->setWindowIcon(QIcon(":/book.jpg"));
*/
    //设置登录对话框大小固定为399*252
    ...
    QPalette palette;
    palette.setBrush(QPalette::Background,QBrush(QPixmap(":/loginbk1.jpg").scaled(this->size())));
    this->setPalette(palette);

    //设置三个按钮的图标
    ui->exitbtn->setIcon(QIcon(":/exit.jpg"));
    ui->registerbtn->setIcon(QIcon(":/reg.jpg"));
    ui->loginbtn->setIcon(QIcon(":/Enter.png"));
    ...
    //设置图片
    ui->piclabel->setPixmap(QPixmap(":/College.png").scaled(ui->piclabel->size()));

    //设置登录按钮不可用
    ui->loginbtn->setEnabled(false);

    //设置lineedit提示语句
    ui->userline->setPlaceholderText("请输入账号");
