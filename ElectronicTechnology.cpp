...
#include "ui_ElectronicTechnology.h"

ElectronicTechnology::ElectronicTechnology(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ElectronicTechnology)
{
    ui->setupUi(this);
    ...
    model->setTable("electonictechnology");
    ...
    _model->setTable("electonictechnology");
    ...
    //设置view不可修改
    ui->electonictechnologyview->setEditTriggers(QAbstractItemView::NoEditTriggers);
    ui->electonictechnologyview->horizontalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
}
...
void ElectronicTechnology::comeStudentManage(QString num)
{
    model->select();
...
//        emit EmitToTeacherManageToChangeStack();
    ...
    ui->electonictechnologyview->setModel(model);
}

void ElectronicTechnology::on_returnbtn_clicked()
{
    this->hide();
    emit EmitToStuManage();
}

void ElectronicTechnology::on_searchbtn_clicked()
{
    ...
    if(buttongroup->checkedId()==0)
    {
        ...
        ui->electonictechnologyview->setModel(_model);
    }
