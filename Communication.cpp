    ...
    ui(new Ui::Communication)
{
    ...
    //设置view不可修改
    ui->communicationview->setEditTriggers(QAbstractItemView::NoEditTriggers);
    ui->communicationview->horizontalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
}
...
void Communication::comeStudentManage(QString num)
{
    model->select();
...
//        emit EmitToTeacherManageToChangeStack();
    ...
    ui->communicationview->setModel(model);
}

void Communication::on_returnbtn_clicked()
{
    this->hide();
    emit EmitToStuManage();
}

void Communication::on_searchbtn_clicked()
{
    ...
    if(buttongroup->checkedId()==0)
    {
        ...
        ui->communicationview->setModel(_model);
    }
